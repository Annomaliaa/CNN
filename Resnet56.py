    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                #Ania F.pad(x[:, :, ::2, ::2]
                #Changed for input tensor, for original model - additional F.pad(x[:, :, ::2, ::2]
                self.shortcut = LambdaLayer(lambda x:
                                            F.pad(x[:, :, ::stride, ::stride], (0, 0, 0, 0, planes//4, planes//4), "constant", 0))
                """
                self.shortcut = LambdaLayer(lambda x:
...
param = torch.load('checkpoints/resnet56-4bfd9763.th', map_location=torch.device('cpu'))
model.load_state_dict(param['state_dict'])

#torch.save(model, 'models/resnet_cifar/resnet56-pre-not-decomp.pth', pickle_module=dill)
...
print("That's for the normal resnet20:")
...
print("That's for the Normal Resnet20:")
flops1, params1 = flopth(model, in_size=((3, 224, 224),), show_detail=True)
...
print("That's for the Decomposed resnet20:")
with profile(activities=[ProfilerActivity.CPU], record_shapes=True) as prof:
    with record_function("model_inference"):
         model(input_tensor)
...
torch.save(checkpoint, 'Results/resnet56-decomp-cp.pth', pickle_module=dill)
