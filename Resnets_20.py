...
param = torch.load('checkpoints/resnet20-12fca82f.th', map_location=torch.device('cpu'))
model.load_state_dict(param['state_dict'])

#torch.save(model, 'models/resnet_cifar/resnet20-pre-not-decomp.pth')
...
dot.format = 'png'
dot.render('resnet20_architecture_not_decomp', cleanup=True)
...
print("That's for the decomp resnet20:")
...
#torch.save(checkpoint, 'Results/resnet20-decomp-cp.pth', pickle_module=dill)

print("That's for the Decomposed Resnet20:")
flops, params = flopth(model, in_size=((3, 16, 16),), show_detail=True)
print(flops, params)
...
dot = make_dot(model(x), params=dict(model.named_parameters()))
dot.format = 'png'
dot.render('resnet20_architecture', cleanup=True)